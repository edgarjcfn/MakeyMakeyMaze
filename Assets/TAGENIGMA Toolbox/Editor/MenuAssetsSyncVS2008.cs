...
/// <summary>
/// Generate VS 2008 compatible solution and project
/// </summary>
[RegisterSymbol(Name = "AS_2GR")]
public class MenuAssetsSyncVS2008 : MenuAssetsSyncVSCommon
{
    /// <summary>
    /// Sync Unity assets into VS 2008
    /// </summary>
    [MenuItem("Assets/Sync Visual Studio 2008")]
    private static void SyncVisualStudio2008()
    {
        try
        {
            ...
            WriteSolutionVisualStudio2008(slnGUID, csprojGUID);
            WriteProjectVisualStudio2008(csprojGUID);

            // get the reload dialog to appear when a file is added
            string fileSuo = string.Format("{0}{1}", currentFolderName, FILE_SUO_2008_SUFFIX);
            ...
            string fileCSProj = string.Format("{0}{1}", currentFolderName, FILE_CSPROJ_2008_SUFFIX);
        ...
        {
            Debug.LogError(string.Format("Failed to sync VS2008 exception={0}", ex));
        }
    }
    ...
    /// </summary>
    /// <returns></returns>
    [MenuItem("Assets/Sync Visual Studio 2008", validate = true)]
    public static bool CheckSyncVS()
    {
        return PlayerPrefs.HasKey(ToolboxEditor.PLAYER_PREFS_KEY_PATH_UNITY);
    }

    private static void WriteSolutionVisualStudio2008(string slnGUID, string csprojGUID)
    {
        try
        {
            DirectoryInfo currentDirectory = new DirectoryInfo(Directory.GetCurrentDirectory());
            ...
            string fileCSProj = string.Format("{0}{1}", currentFolderName, FILE_CSPROJ_2008_SUFFIX);
            projectSection = projectSection.Replace("__CSPROJ_2008__", currentFolderName);
            projectSection = projectSection.Replace("__FILE_CSPROJ_2008__", fileCSProj);
            ...
            string fileSln = string.Format("{0}{1}", currentFolderName, FILE_SLN_2008_SUFFIX);
        ...
        {
            Debug.LogError(string.Format("Failed WriteSolutionVisualStudio2008 exception={0}", ex));
        }
    }

    private const string FILE_SLN_2008_SUFFIX = "-VS2008.sln";
    private const string FILE_CSPROJ_2008_SUFFIX = "-VS2008.csproj";
    private const string FILE_SUO_2008_SUFFIX = "-VS2008.suo";
    private const string FILE_CSPROJ_USER_2008_SUFFIX = "-VS2008.csproj.user";

    private const string TEMPLATE_SOLUTION_PROJECT =
        @"Project(""{__SLN_GUID__}"") = ""__CSPROJ_2008__"", ""__FILE_CSPROJ_2008__"", ""{__CSPROJ_GUID__}""
EndProject";
...
EndGlobal";
    ...
    private static void WriteProjectVisualStudio2008(string csprojGUID)
    {
        try
        {
            DirectoryInfo currentDirectory = new DirectoryInfo(Directory.GetCurrentDirectory());
            ...
            string csproj = TEMPLATE_PROJECT_2008;
            ...
            string fileCSProj = string.Format("{0}{1}", currentFolderName, FILE_CSPROJ_2008_SUFFIX);
            csproj = csproj.Replace("__FILE_CSPROJ_2008__", fileCSProj);
        ...
        {
            Debug.LogError(string.Format("Failed WriteProjectVisualStudio2008 exception={0}", ex));
        }
    }
...
__TEMPLATE_PROJECT_HINT__
    ...
    private const string TEMPLATE_PROJECT_2008 =
