...
/// <summary>
/// Generate VS 2010 compatible solution and project
/// </summary>
[RegisterSymbol(Name = "AS_2GR")]
public class MenuAssetsSyncVS2010 : MenuAssetsSyncVSCommon
{
    /// <summary>
    /// Sync Unity assets into VS 2010
    /// </summary>
    [MenuItem("Assets/Sync Visual Studio 2010")]
    private static void SyncVisualStudio2010()
    {
        try
        {
            ...
            WriteSolutionVisualStudio2010(slnGUID, csprojGUID);
            WriteProjectVisualStudio2010(csprojGUID);

            // get the reload dialog to appear when a file is added
            string fileSuo = string.Format("{0}{1}", currentFolderName, FILE_SUO_2010_SUFFIX);
            ...
            string fileCSProj = string.Format("{0}{1}", currentFolderName, FILE_CSPROJ_2010_SUFFIX);
        ...
        {
            Debug.LogError(string.Format("Failed to sync VS2010 exception={0}", ex));
        }
    }
    ...
    /// </summary>
    /// <returns></returns>
    [MenuItem("Assets/Sync Visual Studio 2010", validate = true)]
    public static bool CheckSyncVS()
    {
        return PlayerPrefs.HasKey(ToolboxEditor.PLAYER_PREFS_KEY_PATH_UNITY);
    }

    private static void WriteSolutionVisualStudio2010(string slnGUID, string csprojGUID)
    {
        try
        {
            DirectoryInfo currentDirectory = new DirectoryInfo(Directory.GetCurrentDirectory());
            ...
            string fileCSProj = string.Format("{0}{1}", currentFolderName, FILE_CSPROJ_2010_SUFFIX);
            projectSection = projectSection.Replace("__CSPROJ_2010__", currentFolderName);
            projectSection = projectSection.Replace("__FILE_CSPROJ_2010__", fileCSProj);
            ...
            string fileSln = string.Format("{0}{1}", currentFolderName, FILE_SLN_2010_SUFFIX);
        ...
        {
            Debug.LogError(string.Format("Failed WriteSolutionVisualStudio2010 exception={0}", ex));
        }
    }

    private const string FILE_SLN_2010_SUFFIX = "-VS2010.sln";
    private const string FILE_CSPROJ_2010_SUFFIX = "-VS2010.csproj";
    private const string FILE_SUO_2010_SUFFIX = "-VS2010.suo";
    private const string FILE_CSPROJ_USER_2010_SUFFIX = "-VS2010.csproj.user";

    private const string TEMPLATE_SOLUTION_PROJECT =
        @"Project(""{__SLN_GUID__}"") = ""__CSPROJ_2010__"", ""__FILE_CSPROJ_2010__"", ""{__CSPROJ_GUID__}""
EndProject";
...
EndGlobal";
    ...
    private static void WriteProjectVisualStudio2010(string csprojGUID)
    {
        try
        {
            DirectoryInfo currentDirectory = new DirectoryInfo(Directory.GetCurrentDirectory());
            ...
            string csproj = TEMPLATE_PROJECT_2010;
            csproj = csproj.Replace("__CSPROJ_GUID__", csprojGUID);

            csproj = csproj.Replace("__DEFINE_CONSTANTS__", GetDefineConstants());
            ...
            string fileCSProj = string.Format("{0}{1}", currentFolderName, FILE_CSPROJ_2010_SUFFIX);
            csproj = csproj.Replace("__FILE_CSPROJ_2010__", fileCSProj);
        ...
        {
            Debug.LogError(string.Format("Failed WriteProjectVisualStudio2010 exception={0}", ex));
        }
    }
...
__TEMPLATE_PROJECT_HINT__
    ...
    private const string TEMPLATE_PROJECT_2010 =
