...
public class GFHexGrid : GFGrid {
	...
	private int[] idx; // the quasi-XYZ indices depending on the grid plane.
	protected Vector3[] units = new Vector3[]{Vector3.right, Vector3.up, Vector3.forward}; // the normal X-, Y- and Z- vectors
	private Vector3[] unitVectors; //right, up and forward relative to the grid's Transform (i.e. in world space)
	private Vector2 slopedUnit;
	...
	public HexOrientation hexSideMode = HexOrientation.PointySides;
	...
	public HexGridShape gridStyle = HexGridShape.Rectangle;
...
//_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-
	...
	public override Vector3 WorldToGrid(Vector3 worldPoint){
		SetHelperValues();
		Vector3 localPoint = cachedTransform.InverseTransformDirection(worldPoint - FindNearestFace(cachedTransform.position, gridPlane));
//		Debug.Log(localPoint);
//		Gizmos.DrawLine(worldPoint, FindNearestFace(cachedTransform.position, gridPlane));
...
//		Debug.Log("strip: " + strip + ", remainder: " + remainder+ ", vert: " + vert);
	...
	//BROKEN for X component  in odd strips (fixed?)
	public override Vector3 GridToWorld(Vector3 gridPoint){
		...
		Vector3 vec = strip % 2 == 0 ? slopedUnit : slopedUnit.GFConjugate(hexSideMode == HexOrientation.FlatSides); // this part is wrong
...
//		Debug.Log(unit);
		...
		worldPoint = worldPoint + FindNearestFace(cachedTransform.position, gridPlane);
	...
	public override Vector3 FindNearestVertex(Vector3 fromPoint, bool doDebug = false){
		...
		Vector3 facePoint = cachedTransform.GFInverseTransformPointFixed(fromPoint) - cachedTransform.GFInverseTransformPointFixed(face); // Vector3 -> Vector2
	...
	public override Vector3 FindNearestFace(Vector3 fromPoint, GridPlane thePlane, bool doDebug = false){
		//SetHelperValues(); //done in the next step already
		...
		toPoint = cachedTransform.GFTransformPointFixed(toPoint);
	...
	protected Vector3 GetBoxCoordinatesHerring(Vector3 world){
		Vector3 local = cachedTransform.GFInverseTransformPointFixed(world);
	...
	protected Vector3 GetVertexCoordinatesSomething(Vector3 world){
		...
		// calculate the offset of the point from the centre of the hex (local 3D coordinates)
		Vector3 facePoint = cachedTransform.GFInverseTransformPointFixed(world) - cachedTransform.GFInverseTransformPointFixed(face); // Vector3 -> Vector2
...
//				Debug.Log ("Shrink by " + relativeScale.x);
	...
	public override void RenderGrid(Vector3 from, Vector3 to, int width = 0, Camera cam = null, Transform camTransform = null){
		RenderGridRect(from, to, width, cam, camTransform);
	}
	
	public void RenderGridRect(int width = 0, Camera cam = null, Transform camTransform = null){
		RenderGridRect(-size, size, width, cam, camTransform);
	}
	#endregion
	
	public void RenderGridRect(Vector3 from, Vector3 to, int width = 0, Camera cam = null, Transform camTransform = null){
		if(!renderGrid)
			return;
		
		//don't let the radius go below the minimum or you get caught in too large loops
	...
	protected Vector3[][][] CalculateDrawPointsRect(Vector3 from, Vector3 to, bool isCompact = false){
		...
		for(int i = 0; i < 3; i++){length[i] = to[i] - from[i];}
		
		//calculate the amount of steps from the centre for the first hex (I will need these values later)
		int startX = Mathf.FloorToInt(to[idx[0]] / (1.5f * radius));
		int startY = Mathf.FloorToInt(to[idx[1]] / height);
		
		int endX = Mathf.CeilToInt(from[idx[0]] / (1.5f * radius));
		int endY = Mathf.CeilToInt(from[idx[1]] / height);
		...
		Vector3[] startPoint = new Vector3[1]{ // can this be expanded to use 3 points and draw incomplete lines like in RectGrid?
			...
			cachedTransform.position + unitVectors[idx[0]] * (1.5f * startX) * radius
				+ unitVectors[idx[1]] * ((startY + (Mathf.Abs(startX % 2)) * 0.5f) * height)
				+ unitVectors[idx[2]] * depth * Mathf.Floor(to[idx[2]] / depth)
		};
		...
		int[] amount = new int[3]{
			startX - endX + 1,
			startY - endY + 1,
			Mathf.FloorToInt(to[idx[2]] / depth) - Mathf.CeilToInt(from[idx[2]] / depth) + 1
		};
		...
		for(int j = 0; j < amount[0]; j++){
			...
			Vector3 hexStart = startPoint[0] + unitVectors[idx[2]] * (to[idx[2]] - depth * Mathf.Floor(to[idx[2]] / depth)); //adjust quasi-Z coordinate
...
//_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-
	...
	protected void SetHelperValues(){
		if(!cachedTransform)
			cachedTransform = transform;
//		width = 2.0f * radius;
		side = 3.0f/2.0f * radius;
		height = Mathf.Sqrt(3.0f) * radius;
		unitVectors = new Vector3[3]{
			cachedTransform.right, cachedTransform.up, cachedTransform.forward
		};
		slopedUnit = new Vector2(1.5f * radius, 0.5f * height);
		Swap<float>(ref slopedUnit.x, ref slopedUnit.y, hexSideMode == HexOrientation.FlatSides);
		idx = TransformIndices();
		Swap<int>(ref idx[0], ref idx[1], hexSideMode == HexOrientation.FlatSides);
	}
	
	//transforms from quasi aexis to real axis. Quasi axis is the relative X, Y and Z n the current grid plane,
	// all calculations are done in quasi space, so there is only one calculation, and then transformed into real space
	protected int[] TransformIndices(){
		int[] idx = new int[3];
		if(gridPlane == GridPlane.YZ){
			idx[0] = 2; idx[1] = 1; idx[2] = (int)gridPlane;
		} else if(gridPlane == GridPlane.XZ){
			idx[0] = 0; idx[1] = 2; idx[2] = (int)gridPlane;
		} else{
			idx[0] = 0; idx[1] = 1; idx[2] = (int)gridPlane;
		}
		return idx;
	}
